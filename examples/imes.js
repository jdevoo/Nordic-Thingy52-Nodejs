...
function getNoble() {
  try {
    ...
    try {
      const thingyPath = require.resolve("../index");
      return require(
        path.join(path.dirname(thingyPath), "node_modules", "noble"),
      );
    } catch {
      return null;
    }
...
async function recordPulse(deviceKey) {
  ...
  try {
    ...
    console.log(`[Pulse] Incremented counter for device: ${deviceKey}`);
  } catch (err) {
...
async function runSession(thingy, deviceKey) {
  // Targeted GATT discovery: only Motion (TMS) and UI (UIS) services needed
  await thingy.connect({
    services: [Thingy.TMS_UUID, Thingy.UIS_UUID],
  ...
  try {
    ...
    await withTimeout(thingy.ui.led.off(), 3000, "LED off").catch(() => {});

    await withTimeout(thingy.ui.button.enable(), 3000, "Button enable");
    await withTimeout(thingy.motion.tap.enable(), 3000, "Tap enable");
    console.log(
      ...
      while (!sigint) {
        try {
          ...
          if (
            ...
            console.log(
              "Connection error. Resetting HCI Bluetooth controller...",
            );
            try {
              const noble = getNoble();
              if (noble && noble._bindings) {
                noble._bindings.reset();
              }
            } catch (_err) {
              // Ignore reset errors
            }
